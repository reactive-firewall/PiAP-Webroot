...
echo -n "<form action=\"/pages/do_wan_setup.php\" method=\"post\" name=\"wan_join_${AP_ADDR}\"><tr><td>"
COL_DELIM="</td><td>"
fi
if [[ ( $USE_HTML -ge 1 ) ]] ; then
if [[ ( -z $AP_KNOWN_NAME ) ]] ; then
echo -n $(fgrep -v "IE: Unknown:" "${SWAP_SCAN_FILE}" | fgrep -m 1 --after-context=6 "${AP_ADDR}" | grep -F -m1 "ESSID:" | cut -d ':' -f 2 | sed -E -e 's/\"\"/<i>Hidden Network<\/i>/g' | sed -E -e 's/\\/\\\\/g' )
else
echo -n "${AP_KNOWN_NAME}"
fi
echo -n "${COL_DELIM}<input type=\"text\" name=\"target_ssid\" id=\"target_ssid\" class=\"form-td\" placeholder=\"WiFi Name\" disabled=\"true\" value=\""
else
...
echo -n $(fgrep -v "IE: Unknown:" "${SWAP_SCAN_FILE}" | fgrep -m 1 "${AP_ADDR}" | tr -s ' ' ' ' | cut -d \  -f 6)
if [[ ( $USE_HTML -ge 1 ) ]] ; then
echo -n "\" />${COL_DELIM}"
else
echo -n "${COL_DELIM}"
...
echo -n "<span class=\"label label-success\">encrypted</span>" ;
else
echo -n "<span class=\"label label-warning\">Open</span>" ;
fi
fi
if [[ ( $USE_HTML -ge 1 ) ]] ; then
echo -n $(fgrep -v "IE: Unknown:" "${SWAP_SCAN_FILE}" | fgrep -m1 --after-context=12 "${AP_ADDR}" | grep -oE "(WPA2|WPA|WEP){1}" | sed -E -e 's/^[WPA]{3}[2]?$/<span class=\"label label-success\">&<\/span>/g' | sed -E -e 's/^[WEP]{3}$/<span class=\"label label-warning\">&<\/span>/g' )"${COL_DELIM}"
else
echo -n $(fgrep -v "IE: Unknown:" "${SWAP_SCAN_FILE}" | fgrep -m1 --after-context=12 "${AP_ADDR}" | grep -oE "(WPA2|WPA|WEP){1}" )"${COL_DELIM}"
fi

echo -n $(fgrep -v "IE: Unknown:" "${SWAP_SCAN_FILE}" | fgrep -m1 --after-context=6 "${AP_ADDR}" | grep -F -m 1 "Frequency:" | cut -d ':' -f 2)"${COL_DELIM}"
echo -n $(fgrep -v "IE: Unknown:" "${SWAP_SCAN_FILE}" | fgrep -m1 --after-context=6 "${AP_ADDR}" | grep -i -F -m 1 "Signal level" | cut -d \= -f 3)

if [[ ( $USE_HTML -le 0 ) ]] ; then
echo ""
else
if [[ ( -n $AP_KNOWN_NAME ) ]] ; then
echo "</td><td><span class=\"label label-success\">Joined</span></td></tr></form>"
elif [[ ( -n $(fgrep -v "IE: Unknown:" "${SWAP_SCAN_FILE}" | fgrep --after-context=8 "${AP_ADDR}" | grep -F -m 1 "Encryption key:" | cut -d ':' -f 2 | fgrep "on" ) ) ]] ; then
echo "</td><td><button class=\"btn btn-lg btn-block btn-off\" id=\"join_button\" onclick=\"javascript:void(0)\" disabled=\"true\">Bully</button></td></tr></form>"
else
echo "</td><td><button class=\"btn btn-lg btn-block btn-primary\" id=\"join_button\" onclick=\"submit\">Join</button></td></tr></form>"
fi
fi
done ;
