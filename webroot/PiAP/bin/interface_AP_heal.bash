...
LOCK_FILE="/tmp/PiAP_interface_Throttle_lock"
...
if [[ ( $(ip addr show ${SOME_IFACE} | grep -oE '\s[s]{1}[tae]{4}\s[A-Z]+\s' | cut -d \  -f 3 ) != "UP" ) ]] ; then
	ifdown --force ${SOME_IFACE} 3>&2 2>/dev/null 1>/dev/null ; wait ;
	ifup ${SOME_IFACE} 2>/dev/null ; EXIT_CODE=$? ; wait ;
	sleep 2;
[[ ( $(ip addr show ${SOME_IFACE} | grep -oE '\s[s]{1}[tae]{4}\s[A-Z]+\s' | cut -d \  -f 3 ) != "UP" ) ]] && EXIT_CODE=255 || EXIT_CODE=0 ;
fi
...
if [[ ( $(ip addr show ${SOME_IFACE} | grep -oE '\s[s]{1}[tae]{4}\s[A-Z]+\s' | cut -d \  -f 3 ) != "UP" ) ]] ; then
	...
	if [[ ( $(openssl dgst -sha512 /etc/hostapd/hostapd.conf | cut -d \  -f 2 ) != $(openssl dgst -sha512 /etc/hostapd/hostapd.conf.backup | cut -d \  -f 2 ) ) ]] ; then
		...
		ifdown --force ${SOME_IFACE} 2>/dev/null > /dev/null ; wait ;
		ifup ${SOME_IFACE} 2>/dev/null ; EXIT_CODE=$? ; wait ;
		sleep 5 ;
		[[ ( $(ip addr show ${SOME_IFACE} | grep -oE '\s[s]{1}[tae]{4}\s[A-Z]+\s' | cut -d \  -f 3 ) != "UP" ) ]] && EXIT_CODE=255 || EXIT_CODE=0 ;
		if [[ ( $(ip addr show ${SOME_IFACE} | grep -oE '\s[s]{1}[tae]{4}\s[A-Z]+\s' | cut -d \  -f 3 ) != "UP" ) ]] ; then
			if [[ ( -f /etc/hostapd/hostapd.conf.failsafe ) ]] ; then
			if [[ ( $(openssl dgst -sha512 /etc/hostapd/hostapd.conf | cut -d \  -f 2 ) != $(openssl dgst -sha512 /etc/hostapd/hostapd.conf.failsafe | cut -d \  -f 2 ) ) ]] ; then
				if [[ ( $(ip addr show ${SOME_IFACE} | grep -oE '\s[s]{1}[tae]{4}\s[A-Z]+\s' | cut -d \  -f 3 ) != "UP" ) ]] ; then
			        	cp -pf /etc/hostapd/hostapd.conf.failsafe /etc/hostapd/hostapd.conf 2>/dev/null || true ;
					ifdown --force ${SOME_IFACE} 2>/dev/null ; wait ;
					ifup ${SOME_IFACE} 2>/dev/null > /dev/null ; EXIT_CODE=$? ; wait ;
					sleep 1;
				fi
			fi
			fi
