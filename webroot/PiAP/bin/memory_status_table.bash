...
echo "<table id="memory_status" class=\"table table-striped\">"
echo "<thead><th>Name</th><th>Total</th><th>Used</th><th>Free</th></thead><tbody>"
free -h | tr -s ' ' ' ' | cut -d\  -f 1-4 | fgrep -v "buffers/cache" | tail -n +2 | sed -E -e 's/(\s*){1}([^ ]+){1}(\s*){1}/<td>\2<\/td>/g' | sed -E -e 's/^(.+){1}$/<tr>\1<\/tr>/g' | sed -E -e 's/(<\/td>){2}/<\/td>/g' | sed -E -e 's/(<td>){2}/<td>/g'
echo "</tbody></table>"
exit 0 ;
